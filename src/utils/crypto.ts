 ...
 * @returns Obj - { password: string, salt: string } - salt + password, both base64 encoded
 */
export const hash = async (pwd: string): Promise<CryptoResponse> => {
  try {
...
export const verify = async (
  ...
  try {
    if (await argon2.verify(params + hashedSalt + "$" + hashedPwd, pwd)) {
